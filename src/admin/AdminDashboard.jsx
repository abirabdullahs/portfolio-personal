...
import WebProjectsAdminPanel from "./WebProjectsAdminPanel";
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 py-10 px-4 font-[Quicksand,Poppins,sans-serif]">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-center mb-8 text-blue-700 dark:text-blue-400">Admin Dashboard</h1>
        <div className="flex justify-center flex-col md:flex-row gap-4 mb-8">
          {["blog", "courses", "products", "webProjects", "messages"].map(t => (
            <button
              key={t}
              className={`px-5 py-2 rounded-lg shadow transition-all ${tab === t ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600"}`}
              onClick={() => setTab(t)}
            >
              {t === "webProjects" ? "Web Projects" : t.charAt(0).toUpperCase() + t.slice(1)}
            </button>
          ))}
        </div>
  ...
  {tab === "messages" && (
          <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-lg rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
            <h3 className="text-xl font-semibold mb-4 text-blue-600 dark:text-blue-400">Contact Messages</h3>
            {messages.length === 0 ? (
              <p className="text-gray-500 dark:text-gray-400">No messages found.</p>
            ) : (
              <ul className="space-y-3">
                {messages.map(m => (
                  <li key={m.id} className="border rounded-lg p-4 bg-gray-50 dark:bg-gray-900 shadow-sm">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{m.timestamp?.toDate?.().toLocaleString?.() || ""}</div>
                      </div>
                      <div className="flex gap-2">
                        <button
                          className={`px-2 py-1 rounded text-xs ${m.read ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                    ...
                    {expandedId === m.id && (
                      <div className="mt-3 p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="whitespace-pre-line text-gray-700 dark:text-gray-300">{m.message}</div>
                      </div>
                    )}
                  </li>
                ))}
  ...
  {tab === "courses" && <CoursesAdminPanel />}
  {tab === "products" && <ProductsAdminPanel />}
  {tab === "webProjects" && <WebProjectsAdminPanel />}
      </div>
    </div>
  );
}
